...
#define vul_foreachptr( T, ref, list ) for ( T **it = ( T** )vul_vector_begin( list ), **first = ( T** )vul_vector_begin( list ), **last = ( T** )vul_vector_end( list ), *ref = ( ( it != last ) ? *it : NULL ); assert( first == ( T* )vul_vector_begin( list ) ), assert( last == ( T* )vul_vector_end( list ) ), it != last; ref = *( ++it ) )
	...
	#undef vul_foreach
		...
		// @Note: Debug versions attempt to spot if you alter the list in the middle of a loop. That might cause a resize and in turn mayhem.
		// Normal iterator
#define vul_foreach( T, it, first, last, list ) for ( it = ( T* )vul_vector_begin( list ), first = ( T* )vul_vector_begin( list ), last = ( T* )vul_vector_end( list ); VUL_ASSERT( first == ( T* )vul_vector_begin( list ) ), VUL_ASSERT( last == ( T* )vul_vector_end( list ) ), it != last; ++it )
		// Easier for list of values; copies
#define vul_foreachval( T, ref, it, first, last, list ) for ( it = ( T* )vul_vector_begin( list ), first = ( T* )vul_vector_begin( list ), last = ( T* )vul_vector_end( list ), ref = ( ( it != last ) ? *it : T( ) ); VUL_ASSERT( first == ( T* )vul_vector_begin( list ) ), VUL_ASSERT( last == ( T* )vul_vector_end( list ) ), it != last; ref = *( ++it ) )
		// Easier for list of pointers
#define vul_foreachptr( T, ref, it, first, last, list ) for ( it = ( T** )vul_vector_begin( list ), first = ( T** )vul_vector_begin( list ), last = ( T** )vul_vector_end( list ), *ref = ( ( it != last ) ? *it : NULL ); VUL_ASSERT( first == ( T* )vul_vector_begin( list ) ), VUL_ASSERT( last == ( T* )vul_vector_end( list ) ), it != last; ref = *( ++it ) )
	#else
		// Normal iterator
